...
class DetectionAlarmUI:
    ...
    def postprocess_output(self, output, number=43, threshold=0.8): 
        ...
        for pred in predictions:
            ...
            if person_confidence > threshold:
                ...
                self.play_alarm()  # Play the alarm sound when detected
                box = pred[0:4]
                detected = True
                break
        if not detected:
            ...
            self.alarm_status.set("Alarm Sound: OFF")
            self.stop_alarm()  # Stop the alarm sound when not detected
        return box
    ...
    def stop_inference(self):
        ...
        self.stop_flag.set()
        self.stop_alarm()  # Stop the alarm sound when inference is stopped
